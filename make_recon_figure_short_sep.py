...
def sample(cfg, logger):
    ...
    def encode(x):
        ...
        Z = model.encode(x, layer_count - 1, 1)
    ...
    def make(paths):
        ...
        with torch.no_grad():
            for filename in paths:
                ...
                f = decode(latents)
                #r = torch.cat([x[None, ...].detach().cpu(), f.detach().cpu()], dim=2)
                canvas.append(f.detach().cpu())
        return canvas

    canvas = make(paths[:16])
    canvas = torch.cat(canvas, dim=0)

    save_image(canvas * 0.5 + 0.5, 'reconstructions_celeba_sep.png', nrow=16)
