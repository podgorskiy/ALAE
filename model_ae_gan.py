...
class Model(nn.Module):
    ...
    def forward(self, x, lod, blend_factor, d_train, ae, alt):
        if ae:
            ...
            Z, d_result_real = self.encode(x, lod, blend_factor)

            Xr = self.generate(lod, blend_factor, Z, mixing=False)
            ...
            if alt:
                with torch.no_grad():
                    Xp = self.generate(lod, blend_factor, count=x.shape[0])
            else:
                with torch.no_grad():
                    Z, _ = self.encode(x, lod, blend_factor)
                    Xp = self.generate(lod, blend_factor, Z, count=x.shape[0])
            ...
            rec = self.generate(lod, blend_factor, z=z.detach(), mixing=False)
