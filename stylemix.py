...
def _main(cfg, logger):
    ...
    for i in range(src_len):
        save_image(src_originals[i] * 0.5 + 0.5, 'style_mixing/source_%d.jpg' % i)
        place(canvas, src_originals[i], 1 + i, 0)

    for i in range(dst_len):
        save_image(dst_originals[i] * 0.5 + 0.5, 'style_mixing/dst_coarse_%d.jpg' % i)
    ...
    for row in range(dst_len):
        ...
        for j in range(rec.shape[0]):
            save_image(rec[j] * 0.5 + 0.5, 'style_mixing/rec_coarse_%d_%d.jpg' % (row, j))
            place(canvas, rec[j], 1 + j, 1 + row)

    save_image(torch.Tensor(canvas), 'style_mixing/stylemix.png')
