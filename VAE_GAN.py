...
from tracker import LossTracker
import math
from model_vae_gan import Model
    ...
    for epoch in range(scheduler.start_epoch(), cfg.TRAIN.TRAIN_EPOCHS):
        ...
        with torch.autograd.profiler.profile(use_cuda=True, enabled=False) as prof:
            for x_orig in tqdm(batches):
                ...
                loss_adv, loss_lae, l_adv_fake = model(x, lod2batch.lod, blend_factor, d_train=True)
                tracker.update(dict(loss_d=loss_adv, loss_adv_fake=l_adv_fake))
                (loss_adv / 200.0 + loss_lae).backward(retain_graph=True)
                encoder_optimizer.step()

                decoder_optimizer.zero_grad()
                loss_adv, loss_lae = model(x, lod2batch.lod, blend_factor, d_train=False)
                tracker.update(dict(loss_g=loss_adv, loss_ae=loss_lae))
                (loss_adv / 200.0 + loss_lae).backward()
